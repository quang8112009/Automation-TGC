...
import { z } from 'zod';
import { validateBody, LoginBodySchema, RegisterBodySchema } from '../http/validation';
...
export async function registerRoutes(app: FastifyInstance, deps: RouteDeps): Promise<void> {
  ...
  app.post('/api/auth/register',
    { config: { rateLimit: AUTH_RATE_LIMIT }, preHandler: [validateBody(RegisterBodySchema)] },
    async (request, reply) => {
      const body = (request as unknown as { validatedBody: { email: string; password: string; name: string } }).validatedBody;
      const result = await authService.register(body as unknown as Record<string, string>);
      return reply.code(201).send(result);
    },
  );

  app.post('/api/auth/login',
    { config: { rateLimit: AUTH_RATE_LIMIT }, preHandler: [validateBody(LoginBodySchema)] },
    async (request, reply) => {
      const body = (request as unknown as { validatedBody: { email: string; password: string } }).validatedBody;
      const result = await authService.login(body.email, body.password);
      return reply.code(200).send(result);
    },
  );

  // Refresh ROTATES the session: it returns a NEW { accessToken, refreshToken }
  // pair and revokes the presenting refresh token's session. Clients MUST replace
  // BOTH stored tokens with the response; replaying the old refresh token now
  // fails with 401 (reuse detection).
  app.post('/api/auth/refresh',
    { preHandler: [validateBody(z.object({ refreshToken: z.string().min(1) }))] },
    async (request, reply) => {
      const body = (request as unknown as { validatedBody: { refreshToken: string } }).validatedBody;
...
async function buildDashboardOverview(
  ...
  const [
    ...
    prisma.scheduledPost.findMany({
      ...
      // Bound the dashboard read: only the most recent failures are surfaced to
      // the UI; an unbounded scan grows with the FAILED table over time.
      take: 50,
    }),
...
async function buildDashboardNotifications(prisma: PrismaClient, actor: AuthInfo) {
  ...
  const [failedPosts, pendingInsights, scheduledPosts] = await Promise.all([
    // Bound notification reads to the most recent items (the FAILED and
    // PENDING_REVIEW tables grow unbounded; the bell only shows a recent slice).
    prisma.scheduledPost.findMany({
      where: { status: 'FAILED' },
      orderBy: { updatedAt: 'desc' },
      take: 50,
