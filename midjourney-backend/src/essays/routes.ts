...
export async function registerEssayRoutes(
  ...
  app.post(
    ...
    { ...governanceRoute({ audit: true }), preHandler: [auth, candidateTargetById('status_update')] },
    async (request, reply) => {
      const actor = getAuth(request);
      const { essayId } = request.params as EssayIdParams;
  ...
  app.delete(
    ...
    { ...governanceRoute({ audit: true, auditAction: 'DATA_DELETE' }), preHandler: [auth, candidateTargetById('update')] },
    async (request, reply) => {
      const actor = getAuth(request);
      const { essayId } = request.params as EssayIdParams;
