 ...
 * draft. Brand assets are batch-rendered in parallel after all drafts succeed,
 * cutting wall-clock time from N×sequential to 1×parallel. Each item is wrapped
 * in try/catch — a Gemini 502 or ANY per-item error becomes a SOFT skip.
 */
function buildGenerateStep(deps: AutopilotDeps): StepDefinition {
  return {
    ...
    run: async (ctx: AgentContext): Promise<AgentResult> => {
      ...
      // Collect draft IDs + asset kinds for batch rendering after all drafts.
      const pendingAssets: Array<{
        kind: AssetKind;
        copy: { title: string; body: string; ctas?: string[]; market?: string };
        draftId: string;
      }> = [];
      ...
      for (const item of items) {
        try {
          ...
          // Queue the brand asset for batch rendering (best-effort).
          // ctas are included via Prisma include at runtime, but the ContentDraft
          // type doesn't expose them — access safely via the runtime object.
          const draftRecord = result.draft as unknown as Record<string, unknown>;
          const draftCtas = Array.isArray(draftRecord.ctas)
            ? (draftRecord.ctas as Array<Record<string, unknown>>)
                .map((c) => (typeof c.ctaText === 'string' ? c.ctaText : ''))
                .filter(Boolean)
            : [];
          pendingAssets.push({
            kind: assetKindForFormat(item.format),
            copy: {
      ...
      // Batch-render all brand assets in parallel (best-effort).
      let assetsRendered = 0;
      let assetsFailed = 0;
      if (pendingAssets.length > 0) {
        try {
          const batchResults = await deps.assetGenerator.generateBatch(pendingAssets, 4);
          for (const r of batchResults) {
            if (r.status === 'RENDERED') assetsRendered += 1;
            else if (r.status === 'FAILED') assetsFailed += 1;
          }
        } catch {
          // Entire batch failed — best-effort, don't fail the step.
          assetsFailed = pendingAssets.length;
        }
      }
