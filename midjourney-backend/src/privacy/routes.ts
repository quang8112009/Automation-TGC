...
import { DSARService } from './dsarService';
...
export function registerPrivacyRoutes(app: FastifyInstance, deps: PrivacyRouteDeps): void {
  ...
  const dsar = new DSARService(prisma, new AuditLogger(prisma));
  ...
  // POST /api/v1/privacy/dsar — Process a DSAR (GDPR Article 15).
  // Returns the complete personal data package for a data subject.
  // ADMIN-only: this endpoint exposes raw PII.
  app.post(
    '/api/v1/privacy/dsar',
    { preHandler: [auth, writeGuard] },
    async (request, reply) => {
  ...
  // GET /api/v1/privacy/dsar — List all DSAR requests (admin audit).
  app.get(
    '/api/v1/privacy/dsar',
    { preHandler: [auth, readGuard] },
    async (request, reply) => {
      const requests = await dsar.listRequests();
      return reply.code(200).send({ items: requests });
    },
  );
}
